...
const typeConfig = {
  bug: { labelKey: "bug", icon: Bug, color: "text-red-400", bg: "bg-red-500/10" },
  suggestion: { labelKey: "suggestion", icon: Lightbulb, color: "text-amber-400", bg: "bg-amber-500/10" },
  other: { labelKey: "other", icon: MessageCircle, color: "text-blue-400", bg: "bg-blue-500/10" },
};

const statusConfig = {
  pending: { labelKey: "pendingStatus", icon: Clock, color: "text-gray-400", bg: "bg-gray-500/10" },
  reviewing: { labelKey: "reviewingStatus", icon: Eye, color: "text-blue-400", bg: "bg-blue-500/10" },
  resolved: { labelKey: "resolvedStatus", icon: CheckCircle, color: "text-emerald-400", bg: "bg-emerald-500/10" },
  closed: { labelKey: "closedStatus", icon: XCircle, color: "text-gray-500", bg: "bg-gray-500/10" },
};
...
export default function AdminFeedbackPage() {
  ...
  const typeLabels: Record<string, string> = {
    bug: t("bugType"),
    suggestion: t("suggestionType"),
    other: t("otherType"),
  };

  const statusLabels: Record<string, string> = {
    pendingStatus: t("pendingStatus"),
    reviewingStatus: t("reviewingStatus"),
    resolvedStatus: t("resolvedStatus"),
  ...
  const fetchFeedbacks = useCallback(async () => {
    ...
    try {
      ...
      showError(tc("error"), err instanceof Error ? err.message : tc("unknownError"));
    } finally {
      setIsLoading(false);
    }
  }, [filterStatus, filterType, router, showError, tc]);
      ...
      <div className="relative max-w-6xl mx-auto px-3 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          >
            ...
            <span className="hidden sm:inline">{t("notificationsLink")}</span>
          </button>
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          ...
          >
            ...
            <option value="suggestion">{typeLabels.suggestion}</option>
          ...
          <div className="space-y-3">
            {feedbacks.length === 0 ? (
              <div className="bg-[var(--bg-secondary)] rounded-xl border border-[var(--border-color)] p-8 text-center">
                ...
                <p className="text-[var(--text-muted)]">{t("noFeedback")}</p>
              </div>
            ) : (
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className={cn("flex items-center gap-1 px-2 py-0.5 rounded-full text-xs", status.bg)}>
                            ...
                            <span className={status.color}>{statusLabels[status.labelKey]}</span>
                          </span>
                        </div>
                      ...
                      {feedback.attachments.length > 0 && (
                        <span className="text-xs text-[var(--text-dimmed)]">
                          {feedback.attachments.length} {feedback.attachments.length !== 1 ? t("attachments") : t("attachment")}
                        </span>
                      )}
                    </div>
                  </button>
          ...
          <div className="lg:sticky lg:top-6 lg:self-start">
            {selectedFeedback ? (
              <div className="bg-[var(--bg-secondary)] rounded-xl border border-[var(--border-color)] overflow-hidden">
                      ...
                      {(() => {
                        ...
                        return (
                          <>
                            ...
                            <span className="font-medium text-[var(--text-primary)]">{typeLabels[type.labelKey]}</span>
                          </>
                        );
                      })()}
                    </div>
                ...
                {/* User Info */}
                <div className="p-4 border-b border-[var(--border-color)]">
                  <p className="text-sm text-[var(--text-dimmed)] mb-1">{t("userLabel")}</p>
                  <p className="font-medium text-[var(--text-primary)]">
                    {selectedFeedback.user.name || t("noNameUser")}
                  </p>
                  <p className="text-sm text-[var(--text-muted)]">{selectedFeedback.user.email}</p>
                  <p className="text-xs text-[var(--text-dimmed)] mt-2">
                    {t("sentAt")} {formatDate(selectedFeedback.createdAt)}
                  </p>
                </div>

                {/* Status Update */}
                <div className="p-4 border-b border-[var(--border-color)]">
                  <p className="text-sm text-[var(--text-dimmed)] mb-2">{t("statusLabel")}</p>
                ...
                {/* Description */}
                <div className="p-4 border-b border-[var(--border-color)]">
                  <p className="text-sm text-[var(--text-dimmed)] mb-2">{t("descriptionLabel")}</p>
                ...
                {selectedFeedback.adminResponse && (
                  <div className="p-4 border-b border-[var(--border-color)] bg-violet-500/5">
                    <p className="text-sm text-[var(--text-dimmed)] mb-2">{t("adminResponse")}</p>
                    <p className="text-[var(--text-primary)] whitespace-pre-wrap">
                      {selectedFeedback.adminResponse}
                    </p>
                    {selectedFeedback.respondedAt && (
                      <p className="text-xs text-[var(--text-dimmed)] mt-2">
                        {t("respondedAt")} {formatDate(selectedFeedback.respondedAt)}
                      </p>
                    )}
                  </div>
                )}
                ...
                <div className="p-4 border-b border-[var(--border-color)]">
                  <p className="text-sm text-[var(--text-dimmed)] mb-2">
                    {selectedFeedback.adminResponse ? t("newResponse") : t("respondToUser")}
                  </p>
                ...
                {selectedFeedback.attachments.length > 0 && (
                  <div className="p-4">
                    <p className="text-sm text-[var(--text-dimmed)] mb-2">{t("attachmentsLabel")}</p>
              ...
              <div className="bg-[var(--bg-secondary)] rounded-xl border border-[var(--border-color)] p-8 text-center">
                ...
                <p className="text-[var(--text-muted)]">{t("selectFeedback")}</p>
              </div>
            )}
          </div>
        </div>
